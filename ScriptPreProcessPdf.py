...
def script_breakdown_pdf(data, file_path, file_path_json, folder, filename, filename_json, scene_location_bank,
                         scene_time_bank, api_call):
    ...
    try:
        ...
        with open(file_name_json, 'wb') as file_obj:
            ...
            file_path_json = file_name_json  # './DataImgn/Download/script_file.docx'
            is_download = True
    ...
    x_flag = False
    try:
        x = json_data['formImage']['Pages'][0]['Texts'][0]['x']
        x_first = json_data['formImage']['Pages'][0]['Texts'][0]['x']
        y = json_data['formImage']['Pages'][0]['Texts'][0]['y']
    except:
        x = 0
        y = 0
        x_first = 0
    ...
    text_style_grouped = ""
    check = 0
    text_style = {'text': "", "style": "", "page": page}
    list_reverse_flag = False
    for element in json_data['formImage']['Pages']:
        page += 1
        for text in element['Texts']:
            if text['y'] > 1:
                ...
                if not x_flag:
                    ...
                    x_flag = True
                if abs(text['y'] - y) > 0.3:
                    y_delta = abs(text['y'] - y)
                    if abs(x_first - text['x']) > 2 or y_delta > 1.25:
                        x_delta = abs(text['x'] - x)
                        x = text['x']
                        y = text['y']
                        x_first = text['x']

                        """
                        if list_reverse_flag:
                            reversed_str = []
                            reversed_str_final = ""
                            reversed_str = text_style['text'].split()
                            i = 0
                            for a_string in reversed_str:
                                reversed_str[i] = reversed_string(a_string)
                                i += 1
                            divided_list = []
                            str_buffer = ""
                            for word in reversed_str:
                                if re.search('\W', word) is not None:
                                    str_buffer += " " + word
                                    divided_list.append(str_buffer.strip())
                                    str_buffer = ""
                                else:
                                    str_buffer += " " + word
                            if str_buffer != "":
                                divided_list.append(str_buffer)
                            for str_buffer in divided_list:
                                str_list = str_buffer.split()
                                if len(str_list) > 1:
                                    if re.search('\W', str_buffer) is not None:
                                        sign = re.findall('\W', str_buffer)
                                        sign_str = ""
                                        for s in sign:
                                            sign_str += s
                                        sign_str = sign_str.strip()
                                        str_buffer = re.sub('\W', " ", str_buffer)
                                        str_list = str_buffer.split()
                                        str_list = str_list[::-1]
                                        str_list.append(sign_str)
                                    else:
                                        str_list = str_list[::-1]
                                for word in str_list:
                                    reversed_str_final += " " + word
                            reversed_str_final = reversed_str_final.strip()
                            text_style = {'text': reversed_str_final, 'style': 'text', 'page': page}
                            list_reverse_flag = False
                        """
                        pdf_dict[index] = text_style
                        index += 1

                        text_style = {'text': "", "style": "", "page": page}
                        text_str = json.dumps(text['R'][0]['T'], ensure_ascii=False, sort_keys=True)
                        if text_str.count('%') * 4 >= len(text_str):
                            rtl_flag = True
                            rtl_counter += 1
                            list_reverse_flag = True
                        text_str_fixed = json.loads(text_str, encoding="UTF-8")
                        text_str_fixed = urllib.parse.unquote(text_str_fixed, encoding='UTF-8')

                        if x_delta < 0.7:
                            text_str_fixed = text_str_fixed.strip()
                            text_style['text'] += text_str_fixed.strip('"')
                            x = text['x']
                        else:
                            if end_with_space(text_str_fixed):
                                text_str_fixed = text_str_fixed.strip()
                                if len(text_str_fixed) > 1:
                                    text_style['text'] += text_str_fixed.strip('"') + " "
                                else:
                                    text_style['text'] += text_str_fixed.strip('"')
                                x = text['x']
                                ...
                                if len(text_str_fixed) > 1:
                                    text_style['text'] += " " + text_str_fixed.strip('"')
                                    text_style_grouped += text_str_fixed.strip('"')
                                    check += 1
                                    if text_style_grouped in loc_bank_sorted or text_style_grouped in time_bank_sorted:
                                        if check == 2:
                                            text_style_fixed = text_style['text'].split()
                                            text_style_temp = []
                                            for cou in range(len(text_style_fixed) - 2):
                                                text_style_temp.append(text_style_fixed[cou])
                                            text_style_temp.append(text_style_grouped)
                                            text_style['text'] = ""
                                            for word in text_style_temp:
                                                text_style['text'] += " " + word
                                            text_style['text'] = text_style['text'].strip()
                                            text_style_grouped = ""
                                            check = 0
                                        else:
                                            if check > 2:
                        ...
                        x_delta = abs(text['x'] - x)
                        text_str = json.dumps(text['R'][0]['T'], ensure_ascii=False)
                        if text_str.count('%') * 4 >= len(text_str):
                            rtl_flag = True
                            list_reverse_flag = True
                            rtl_counter += 1
                                ...
                                if text_style_grouped in loc_bank_sorted or text_style_grouped in time_bank_sorted:
                                    if check == 2:
                                        ...
                                        if check > 2:
                                            text_style_grouped = ""
                                            check = 0
                                        if re.search('\W', text_style_grouped) != None:
                                            text_style_grouped = ""
                                            check = 0
                                else:
                                    if check > 1:
    ...
    print("rtl_counter", rtl_counter)
    ltr_counter = total_elm_counter - rtl_counter
    if rtl_counter > ltr_counter:
        ...
        for element in line_data:
            ...
            if len(element) < 15:
                for word in element:
                    element_buffer.append(word)
                pdf_dict[index2] = {'text': element_buffer, 'style': 'text', 'page': 0}
                element_buffer = []
            else:
                for word in element:
                    element_buffer.append(word)
    ...
    pdf_dict_fixed = {}
    if rtl_counter > ltr_counter:
        for i, element in pdf_dict.items():
            pdf_dict_fixed[index2] = {'text': element['text'], 'style': 'text', 'page': element['page']}
            index2 += 1
    else:
        for i, element in pdf_dict.items():
            pdf_dict_fixed[index2] = {'text': element['text'].split(), 'style': 'text', 'page': element['page']}
            index2 += 1

    result1 = pdf_dict_fixed
    ...
    print("result1_fixed", result1fixed)
    flag_predict = False
    ...
    # here we clean scene title thats ends with num from num
    for i, element in result1fixed.items():
        if element['style'] == scenekey:
            element['text'][len(element['text']) - 1] = re.sub('\d', '', element['text'][len(element['text']) - 1])

    # here we remove last word if it is empty
    for i, element in result1fixed.items():
        if element['style'] == scenekey:
            if element['text'][len(element['text']) - 1] == "" or element['text'][len(element['text']) - 1] == "()":
                temp_list = element['text']
                element['text'] = []
                for j in range(len(temp_list) - 1):
                    element['text'].append(temp_list[j])
    ...
    print("loc_bank_sorted", loc_bank_sorted)
    for i, element in result1fixed.items():
        if element['style'] == scenekey:
            try:
                word0 = re.sub('[.,-]', "", element['text'][0])
    ...
    for k, element in result1fixed.items():
        ...
        if element['style'] == scenekey:
            ...
            element['text'] = scene_title_str.split()
            print('element text fixed', element['text'])
    ...
    for i in range(0, len(first_scene_title_word_array), 2):
        ...
        if loc_test == None or loc_test2 == None:
                    ...
                    if loc0 in loc_bank_array_nc or loc1 in loc_bank_array_nc:
                        ...
                        try:
                            if loc_del == '-' or loc_del == '–':
                                ...
                                """
                                if loc_del in loc0 or loc_del in loc1 or loc_del in loc2:
                                    element['style'] = scenekey
                                    print("scene found", element['text'])
                                """
                                if loc_del in scene_title_str:
                                    element['style'] = scenekey
                                    print("scene found", element['text'])
                        except:
                            pass
    ...
    # here we check if bank is ready
    bank_ok = True
    count_not_a_scene = 0
    for i, element in result1fixed.items():
        if element['style'] == scenekey:
        ...
        if element['style'] == scenekey:
            ...
            try:
                if text_array[0] in loc_bank_sorted and text_array[2] in loc_bank_sorted:
                    ...
                    print("fixed location", result1fixed[i]['text'])
            ...
            for word in result1fixed[j]['text']:
                ...
                if c == len(result1fixed[j]['text']) - 1:
                    for timecheck in last_s_word_set:
                        try:
                            temp = re.search(timecheck.casefold(), word.casefold())
                            if temp != None:
                                time = temp.group()
                        except:
                            pass
    ...
    while j < len(result1fixed):
        ...
        if result1fixed[j]['style'] == scenekey:
            ...
            if script != []:
                    ...
                    for z in range(len(empty_secne_name)):
                        scenearray.append(
                            {'type': "scene", 'scene_id': empty_scene_scnid[z], 'chapter_number': chapter_number,
                             ...
                             'script': script, 'eighth': eighth_num, 'page': page_num,
                ...
                if scnid != "999999":
                    ...
                    scenearray.append({'type': "scene", 'scene_id': scnid, 'chapter_number': chapter_number,
                                       ...
                                       'param_text': scene_param, 'text': synopsis, 'extras': 0, 'extras_text': "",
                                       'bits': 0, 'bits_text': "", 'one_shoot': 0, 'reshoot': 0, 'prepare': 0,
                                       'script': script, 'eighth': eighth_num, 'synopsis': synopsis, 'page': page_num,
                                       'sub_scenes_array': sub_scenes_array})
                    empty_scene_scnid = []
                    empty_secne_name = []
                    empty_scene_loc = []
            ...
            if loc == "":
                for word in first_s_word_set:
                    name = name.strip()
                    if findWholeWord(word)(name.casefold()) != None:
                        loc = word
                        name = re.sub(loc, "", name)
            if time == "":
                for word in last_s_word_set:
                    try:
                        if findWholeWord(word)(name.casefold()) != None and len(time) < len(word):
                            time = word
                    except:
                        pass
            else:
                if findWholeWord(time)(name.casefold()) != None:
        ...
        if j == len(result1fixed) - 1:
            ...
            scenearray.append({'type': "scene", 'scene_id': scnid, 'chapter_number': chapter_number,
                               ...
                               'param_text': scene_param, 'text': synopsis, 'extras': 0, 'extras_text': "",
                               'bits': 0, 'bits_text': "", 'one_shoot': 0, 'reshoot': 0, 'prepare': 0,
                               'eighth': eighth_num, 'page': page_num,
    ...
    dictforjson.update({'loc_del': loc_del})
    dictforjson.update({'time_del': time_del})
    print("first_s_word_array", first_s_word_array)
    print("rtl_counter", rtl_counter)
    print("ltr_counter", ltr_counter)
